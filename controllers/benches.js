...
module.exports.createBench = async (req, res) => {
    ...
    if (req.files) {
        ...
        bench.images = req.files.map(f => ({ url: f.path, filename: f.filename }));
    }
    ...
    try {
        ...
        if (!geolocation.length) throw new ExpressError('Invalid bench address', 400);
        // Set formatted address and coordinates of bench
        bench.address = geolocation[0].formattedAddress;
        bench.location = { type: 'Point', coordinates: [geolocation[0].longitude, geolocation[0].latitude] };
    } catch (e) {
    ...
    await bench.save();
    // If bench was saved, remove bench description from session variables
    delete req.session.description;
...
module.exports.showBench = async (req, res,) => {
    ...
    const bench = await Bench.findById(id).populate('author').populate({
        path: 'reviews',
        populate: {
            path: 'author'
        }
...
module.exports.editBench = async (req, res) => {
    ...
    if (req.body.deleteImages) {
        ...
        await bench.updateOne({ $pull: { images: { filename: { $in: req.body.deleteImages } } } })
    }
